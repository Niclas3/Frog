                        ...
                        struct dir_entry *target_entry)
{
        ...
        // 12 direct access i_zones[] -> 12 addresses           -> 12 * 4 = 48
        // bytes 1  1-layer access          -> 512 / 4 = 128 addresses-> 512
        // bytes
        ...
        // 512/sizeof(struct dir_entry) dir_entries. Testing all dir_entries
        // find entry name equals given name.
                       ...
                       void *io_buf)
{
        ...
        for (int zones_idx = 0; (zones_idx < 140); zones_idx++) {
                // No find accessible entry, need to allocate new zone for new
                // entry
                if (all_zones[zones_idx] == 0) {
                        ...
                        if (zones_idx < 12) {  // direct table
                                p_dir->inode->i_zones[zones_idx] =
                                    all_zones[zones_idx] = zone_lba;
                                ...
                                if (zone_lba == -1) {
                                        // unset zone bitmap
                                        uint_32 bitmap_idx =
                                            p_dir->inode->i_zones[12] -
                                            part->sb->s_data_start_lba;
                                        set_value_bitmap(&part->zone_bitmap,
                                                         bitmap_idx, 0);
                                ...
                                memset(io_buf, 0, 512);
                                memcpy(io_buf, new_entry,
                                       sizeof(struct dir_entry));
                                ide_write(part->my_disk, all_zones[zones_idx],
                                          io_buf, 1);
                                p_dir->inode->i_size +=
                                    sizeof(struct dir_entry);
                                return 0;
                        ...
                        ide_read(part->my_disk, all_zones[zones_idx],
                                 entries_buf, 1);
                        for (entry_idx = 0;
                             entry_idx < (512 / sizeof(struct dir_entry));
                             entry_idx++) {
                                if ((strlen(entries_buf[entry_idx].filename) ==
                                     0) &&
                                    entries_buf[entry_idx].f_type ==
                                        FT_UNKOWN) {
                                        // Find accessible entry index at
                                        // entry_idx
                                        memcpy(&entries_buf[entry_idx],
                                               new_entry,
                                               sizeof(struct dir_entry));
                                        ide_write(part->my_disk,
                                                  all_zones[zones_idx],
        ...
        for (zone_idx = 0; zone_idx < MAX_ZONE_COUNT; zone_idx++) {
                ...
                if (found_entry && zone_link == 0) {
                        ...
                        if (zone_idx < 12) {
                                ...
                                // If has other zones in indirect table then not
                                // recycle indirect table
                                if (indirect_zones_cnt > 1) {
                                        all_zones[zone_idx] = 0;
                                        ide_write(part->my_disk,
                                                  pdir->inode->i_zones[12],
                                                  all_zones + 12, 1);
                                } else {  // if only has one (include current
                                          // entry) then recycle all indirect
                                          // table
                                        zone_bitmap_idx =
                                            pdir->inode->i_zones[12] -
                                            part->sb->s_data_start_lba;
                        ...
                        ide_write(part->my_disk, all_zones[zone_idx], entries,
                                  1);
                }
