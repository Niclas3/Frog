...
static void partition_format(struct partition *part)
{
        ...
        uint_32 used_sector_sz = os_boot_record_sz + super_block_sz +
                                 inode_bitmap_sz + inode_table_sz;
        ...
        // A = B / U(zone size)
        /* uint_32 zone_bitmap_sz = DIV_ROUND_UP(free_sector_sz,
         * BITS_PER_SECTOR+1);
         */
        ...
        buf_sz =
            ((buf_sz >= sb.s_inode_table_sz) ? buf_sz : sb.s_inode_table_sz) *
            SECTOR_SIZE;
        ...
        ide_write(part->my_disk, sb.s_inode_table_lba, buf,
                  sb.s_inode_table_sz);
...
static bool mount_partition(struct list_head *ele, int arg)
{
        ...
        mounted_part.inode_bitmap.map_bytes_length =
            sb->s_imap_sz * SECTOR_SIZE;
...
static int_32 search_file_recur(struct partition *part,
                                ...
                                const char *name)
{
        // Read this_dir->inode->i_zones[] for all dir_entry and dig into
        // directory at this_dir
        ...
        for (int zones_idx = 0; zones_idx < 140; zones_idx++) {
                ...
                for (int entry_idx = 0;
                     entry_idx < (512 / sizeof(struct dir_entry));
                     entry_idx++) {
                        ...
                        if (cur_entry.f_type == FT_DIRECTORY) {
                                ...
                                int_32 inode_nr =
                                    search_file_recur(part, next_d, name);
                                dir_close(next_d);
                                sys_free(entries_buf);
                                return inode_nr;
...
int_32 sys_open(const char *pathname, uint_8 flags)
{
        ...
        if (depth > 1) {
                ...
                for (int_32 idx = (depth - 2) * MAX_FILE_NAME_LEN; idx >= 0;
                     idx -= MAX_FILE_NAME_LEN) {
                        ...
                        int_32 d_inode =
                            search_file(&mounted_part, next_dir, cur_name);
                        if (d_inode == -1) {
                                // TODO:
                        ...
                        dir_close(next_dir);  // dir_close() does not close root
                                              // directory
        ...
        case O_CREAT: {
                if (file_inode_nr != -1) {
                        if (is_char_file(&mounted_part, file_inode_nr)) {
                                ...
                                // create client
                                fd = open_pkg(&mounted_part, next_dir,
                                              last_name, file_inode_nr, flags);
                        } else {
                                fd = file_create(&mounted_part, next_dir,
                                                 last_name, flags);
                        }
                } else {
        ...
        case O_WRONLY:
                ...
                {
                        if (is_char_file(&mounted_part, file_inode_nr)) {
                                if (is_dev(&mounted_part, file_inode_nr,
                                           DNOAUX)) {
                                        fd = open_aux(&mounted_part,
                                                      file_inode_nr, flags);
                                } else if (is_dev(&mounted_part, file_inode_nr,
                                                  DNOLFB)) {
                                        fd = open_lfb(&mounted_part,
                                                      file_inode_nr, flags);
                                } else {
                                        fd = open_aux(&mounted_part,
                                                      file_inode_nr, flags);
                                ...
                                fd = file_open(&mounted_part, file_inode_nr,
                                               flags);
                        }
                        break;
                }
...
int_32 sys_write(int_32 fd, const void *buf, uint_32 count)
{
        ...
        if (fd == FD_STDOUT_NO) {
                ...
                if (wr_file->fd_flag & O_WRONLY || wr_file->fd_flag & O_RDWR) {
                        bytes_written =
                            file_write(&mounted_part, wr_file, buf, count);
                        return bytes_written;
                } else {
                        // TODO:
                        // put string
                        // put_str("sys_write: not allowed to write file without
                        // flag O_RDWR or
                        //_WRONLY\n");
                        return -1;
                }
...
int_32 sys_mkdir(const char *pathname)
{
        ...
        if (strlen(parent_path) == 1 && parent_path[0] == '/') {
                ...
                pdir_inode_nr =
                    search_file_with_pathname(part, parent_path, &ppdir);
                if (pdir_inode_nr == -1) {
                        // TODO:
        ...
        new_dir_entry(last_name, new_dir_inode_nr, FT_DIRECTORY,
                      &cur_dir_entry);
...
char *sys_getcwd(char *buf, int_32 size)
{
        ...
        while ((cwd_inode_nr)) {
                parent_inode_nr =
                    get_parent_dir_inode_nr(part, cwd_inode_nr, io_buf);
...
int_32 sys_chdir(const char *pathname)
{
        ...
        if (inode_nr != -1) {
                ...
                if (((target_inode->i_mode >> 11) & 0x0000000f) ==
                    FT_DIRECTORY) {
...
int_32 sys_stat(const char *pathname, struct stat *statbuf)
{
        ...
        if (!strcmp(pathname, "/") || !strcmp(pathname, "/.") ||
            !strcmp(pathname, "/..")) {
                ...
                int_32 idx;
                for (idx = 0;
                     root_dir.inode->i_zones[idx] && idx < MAX_ZONE_COUNT;
                     idx++)
                        ;
                ...
                for (int_32 idx = (depth - 2) * MAX_FILE_NAME_LEN; idx >= 0;
                     idx -= MAX_FILE_NAME_LEN) {
                        ...
                        if (d_inode == -1) {
                                ...
                                d_inode = search_file(&mounted_part, next_dir,
                                                      cur_name);
                                if (d_inode == -1) {
                                        // TODO:
                                        //  second check error
