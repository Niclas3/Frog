                  ...
                  uint_32 write_len)
{
        ...
        // 2. Test this file last writable zone. To calculate how many zones
        // this time needs.
        ...
        if (need_new_zone_lba && !all_zones[zone_idx]) {
                // zone_idx works like file position, always find next empty
                // slot of zones 140 is max zones count
                for (zone_idx = 0;
                     all_zones[zone_idx] && zone_idx < MAX_ZONE_COUNT;
                     zone_idx++)
                        ;
                rest_sz = f_inode->i_size % ZONE_SIZE;
        }

        // Ready to be written block of data is chunk after filling the rest
        // zone how many zones this time need
        ...
        if (rest_sz == 0) {
                ...
                if (count <= need_filled_sz) {
                        ...
                        chunk_cnt =
                            DIV_ROUND_UP(count - need_filled_sz, ZONE_SIZE);
                }
        }
        ...
        for (int i = 0; i < chunk_cnt; i++) {
                ...
                if (!all_zones[zone_idx + i]) {
                        ...
                        if (zone_lba == -1) {
                                // TODO:
                                // kprint("No enough zone count when write a
                                // file");
                                sys_free(io_buf);
                                return -1;
                        }
                        ...
                        if (zone_idx + i < 12) {
                                ...
                                if (zone_idx + i == 12) {
                                        ...
                                        f_inode->i_zones[zone_idx + i] =
                                            zone_lba;
                                        // the zone_lba was taken to be save
                                        // indirect table we need a new zone_lba
                                        // for saving data.
                                        zone_lba = zone_bitmap_alloc(part);
                                        if (zone_lba == -1) {
                                                // TODO:
                                                // kprint("No enough zone count
                                                // when write a file");
                                                sys_free(io_buf);
                                                return -1;
                                        }
                                        flush_bitmap(
                                            part, ZONE_BITMAP,
                                            zone_lba -
                                                part->sb->s_data_start_lba);
                                }
                                uint_32 *table =
                                    (uint_32 *) sys_malloc(ZONE_SIZE);
                                if (!table) {
                                        // TODO:
                                        // kprint("No enough zone count when
                                        // write a file");
                                        return -1;
                                }
                                ide_read(part->my_disk, f_inode->i_zones[12],
                                         table, 1);
                                table[zone_idx - 12 + i] = zone_lba;
                                ide_write(part->my_disk, f_inode->i_zones[12],
                                          table, 1);
                                sys_free(table);
                        }
                        // if zone has data already aka (it has a zone_lba aka
                        // all_zones[zone_idx] != 0) write a zone buffer to this
                        // zone_lba
                        ASSERT(w_cursor - (uint_8 *) buf <= count);
                        // If there is a rest chunk and this ture is the last
                        // turn
                        bool is_last_turn = (i == (chunk_cnt - 1));
                        int_32 ret =
                            write_zone(part, file, chunk_rest, zone_lba,
                        ...
                        ASSERT(w_cursor - (uint_8 *) buf <= count);
                        bool is_last_turn = (i == (chunk_cnt - 1));
                        int_32 ret = rewrite_zone(part, file, chunk_rest,
                                                  zone_lba, is_last_turn,
                                                  &w_cursor, &bytes_written);
                        if (ret == -1)
                                return -1;
                        // If there is a rest chunk and this ture is the last
                        // turn
