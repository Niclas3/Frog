...
/*         #<{(|     (struct thread_stack *) ((uint_32) child_thread + PAGE_SIZE - |)}># */
/*         #<{(|                              sizeof(struct context_registers) - |)}># */
...
/*                 return -1; */
/*         } */
/*         #<{(| ASSERT((INTR_OFF == intr_get_status()) && parent_thread->pgdir != */
/*          * NULL); |)}># */
/*         #<{(| */
/*          * Remove intr_get_status() because when sys_fork() called. irq is */
/*          *always disable? */
/*          ****************************************************************************|)}># */
...
/*  */
/*         ASSERT( */
/*             !list_find_element(&child_thread->all_list_tag, &thread_all_list)); */
/*         list_add_tail(&child_thread->all_list_tag, &thread_all_list); */
/*  */
/*         ASSERT( */
/*             !list_find_element(&child_thread->general_tag, &thread_ready_list)); */
