...
uint_32 ioqueue_put_data(CircleQueue *queue, char *data, uint_32 count)
{
        ...
        while (written < count) {
                ...
                if (ioqueue_avaliable(queue) > count) {
                        // write data to queue
                        while (ioqueue_avaliable(queue) > 0 &&
                               written < count) {
                                queue->buf[queue->producer_p] = *w_data;
                                w_data++;
                                written++;
                                queue->producer_p =
                                    next_index(queue, queue->producer_p);
                        }
                }
                ...
                if (ioqueue_unread(queue) >= count) {
                        while (ioqueue_unread(queue) > 0 && read_size < count) {
                                *rd_cursor = queue->buf[queue->consumor_p];
                                ...
                                read_size++;
                                queue->consumor_p =
                                    next_index(queue, queue->consumor_p);
                        }
                }
...
{
        int_32 pg_count = 1;
        if (size > PAGE_SIZE + sizeof(CircleQueue)) {
                pg_count = DIV_ROUND_UP(size, PAGE_SIZE + sizeof(CircleQueue));
        }
