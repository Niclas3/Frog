...
{
    int_16 deltaX = ABS((end.X - start.X));    // Delta X, change in X values, positive absolute value
    int_16 deltaY = -ABS((end.Y - start.Y));   // Delta Y, change in Y values, negative absolute value
    int_16 signX = (start.X < end.X) ? 1 : -1; // Sign of X direction, moving right (positive) or left (negative)
    int_16 signY = (start.Y < end.Y) ? 1 : -1; // Sign of Y direction, moving down (positive) or up (negative)
...
//  Adapted from "Computer Graphics Principles and Practice in C 2nd Edition"
//void draw_ellipse(Point center, uint_16 radiusX, uint_16 radiusY, uint_32 color)
//{
...
void boundary_fill(uint_16 X, uint_16 Y, uint_32 fill_color, uint_32 boundary_color)
{
    // Recursive - may use a lot of stack space
    uint_8 *framebuffer    = (uint_8 *)g_gfx_mode->physical_base_pointer; 
    uint_8 bytes_per_pixel = (g_gfx_mode->bits_per_pixel+1) / 8;             // Get # of bytes per pixel, add 1 to fix 15bpp modes
    uint_8 draw = 0;
...
void fill_polygon_solid(Point vertex_array[], uint_8 num_vertices, uint_32 color)
{
    Point temp;
...
// TODO: Put this back in later, using non-floating point values/ROUND if possible
//void fill_ellipse_solid(Point center, uint_16 radiusX, uint_16 radiusY, uint_32 color)
//{
